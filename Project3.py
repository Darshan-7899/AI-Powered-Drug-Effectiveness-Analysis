...
sns.countplot(x='Drug_Name', data=df,palette='pastel')
...
plt.figure(figsize=(10,4))
sns.countplot(x='Side_Effect_List', data=df, color='lightgreen')  
plt.xticks(rotation=45)
...
sns.histplot(df['Effectiveness_Rating'], bins=10, kde=True, color='orange')  # Changed color to orange
...
sns.boxplot(x='Drug_Name', y='Effectiveness_Rating', data=df,palette = 'pastel')
plt.xticks(rotation=45)
...
sns.boxplot(x='Gender', y='Effectiveness_Rating', data=df, palette = 'pastel')
...
plt.figure(figsize=(8,4))
sns.boxplot(x='age_group', y='Effectiveness_Rating', data=df, palette='Set2')
...
# feature between Drug_Name and Effectiveness_Rating
le = LabelEncoder()
